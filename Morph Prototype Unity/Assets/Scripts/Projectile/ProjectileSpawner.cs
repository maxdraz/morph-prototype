...
public class PositionAndDirection
{
    ...
    public PositionAndDirection(Vector3 spawnPosition, Vector3 direction)
    {
...
public class ProjectileSpawner : MonoBehaviour
{
    ...
    [SerializeField] private List<PositionAndDirection> projectileSpawnData;
            ...
            if (loop)
            {
                foreach (var positionAndDirection in projectileSpawnData)
                {
                    ...
                    var endPos = transform.TransformPoint(positionAndDirection.SpawnPosition)
                                 + transform.TransformDirection(positionAndDirection.Direction);
    ...
    private void DrawEdgeModeGizmos()
    {
        ...
        foreach (var projectileSpawnInfo in projectileSpawnData)
        {
            ...
            Gizmos.DrawLine(
                ...
                transform.TransformPoint(projectileSpawnInfo.SpawnPosition)
                + transform.TransformDirection(projectileSpawnInfo.Direction));
    ...
    private void DrawRadialModeGizmos()
    {
        ...
        foreach (var positionAndDirection in projectileSpawnData)
        {
            ...
            Gizmos.DrawLine(transform.TransformPoint(positionAndDirection.SpawnPosition),
                transform.TransformPoint(positionAndDirection.SpawnPosition) +transform.TransformDirection(positionAndDirection.Direction));
        }
    }
    ...
    private void RecalculateProjectileSpawnInfo()
    {
        ...
        if (spawnMode == ProjectileSpawnMode.Edge)
        {
            for (int i = 0; i < projectilesToSpawn; i++)
            {
                ...
                projectileSpawnData.Add(new PositionAndDirection(spawnPoint, (endPoint - spawnPoint).normalized));
            }
        ...
        for (int i = 0; i < projectilesToSpawnRadially; i++)
        {
            ...
            projectileSpawnData.Add(new PositionAndDirection(spawnPos,(spawnDir - spawnPos).normalized));
        }
